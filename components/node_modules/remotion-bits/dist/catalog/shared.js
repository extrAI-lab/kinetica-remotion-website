import { sharedBitInventory } from "./inventory.generated.js";
const normalizeText = (value) => value.trim().toLowerCase();
const tokenizeQuery = (value) => Array.from(new Set(normalizeText(value).split(/\s+/).filter(Boolean)));
const cloneSummary = (entry) => ({
    ...entry,
    tags: [...entry.tags],
    componentNames: [...entry.componentNames],
    registryDependencies: [...entry.registryDependencies],
});
const normalizeTags = (tags) => Array.from(new Set((tags ?? []).map((tag) => normalizeText(tag)).filter(Boolean)));
...
const scoreBitCatalogEntry = (entry, query) => {
    if (!query) {
        return 0;
    }
...
const catalogSummaries = sharedBitInventory.map((entry) => ({
...
export const listBitCatalog = () => catalogSummaries.map((entry) => cloneSummary(entry));
export const getBitCatalogSummaries = () => listBitCatalog();
export const getBitCatalogTags = () => Array.from(new Set(catalogSummaries.flatMap((entry) => entry.tags))).sort((left, right) => left.localeCompare(right));
...
export const resolveBitCatalogIdentifier = (identifier) => {
    ...
    const nameMatches = catalogSummaries.filter((entry) => normalizeText(entry.name) === normalizedIdentifier);
