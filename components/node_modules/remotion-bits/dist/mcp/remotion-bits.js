...
import { fetchBit, findBits } from "../catalog/runtime.js";
const require = createRequire(import.meta.url);
const packageJson = require('../../package.json');
const isRecord = (value) => typeof value === 'object' && value !== null && !Array.isArray(value);
const assertString = (value, fieldName) => {
    if (typeof value !== 'string' || value.trim().length === 0) {
        throw new McpError(ErrorCode.InvalidParams, `The ${fieldName} field must be a non-empty string.`);
    }
    return value;
};
...
const assertOptionalTags = (value) => {
    if (value === undefined) {
        return undefined;
    }
...
const assertOptionalLimit = (value) => {
    if (value === undefined) {
        return undefined;
    }
...
const parseFindArguments = (value) => {
    if (value === undefined) {
        return {};
    }
    if (!isRecord(value)) {
        throw new McpError(ErrorCode.InvalidParams, 'find_remotion_bits arguments must be an object.');
    }
    return {
...
const parseFetchArguments = (value) => {
    if (!isRecord(value)) {
        throw new McpError(ErrorCode.InvalidParams, 'fetch_remotion_bit arguments must be an object.');
    }
    return {
        id: assertString(value.id, 'id'),
    };
};
const toTextContent = (payload) => [
    {
        type: 'text',
...
const toolDefinitions = [
    ...
    },
    {
        ...
        description: 'Fetch one live Remotion Bit by id, including metadata, source path, and source code.',
        annotations: {
            title: 'Fetch Remotion Bit',
            readOnlyHint: true,
            destructiveHint: false,
...
export const createRemotionBitsMcpServer = () => {
    const server = new Server({
        ...
        instructions: 'Use find_remotion_bits first, then fetch_remotion_bit for the best one or two matches. Adapt examples before composing from primitives.',
    });
...
export const startRemotionBitsMcpServer = async () => {
