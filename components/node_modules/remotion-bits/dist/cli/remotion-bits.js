...
import { fetchBit, findBits, resolveBitCatalogIdentifier } from "../catalog/runtime.js";
import { startRemotionBitsMcpServer } from "../mcp/remotion-bits.js";
...
const collectTags = (values) => dedupeStrings(values
...
const parseLimit = (value) => {
    ...
    if (!Number.isInteger(limit) || limit < 1) {
        throw new CliError('The --limit value must be a positive integer.', EXIT_INVALID_INPUT, 'invalid-limit');
    }
    return limit;
};
const serializeSummary = (entry) => ({
    id: entry.id,
...
const formatTags = (tags) => (tags.length > 0 ? tags.join(', ') : 'none');
const formatDimensions = (entry) => {
    if (entry.width === undefined || entry.height === undefined) {
        return 'auto';
    }
    return `${entry.width}x${entry.height}`;
};
const formatSummaryBlock = (entry, index) => {
...
const formatEntryBlock = (entry) => [formatSummaryBlock(entry), '   Source code:', entry.sourceCode].join('\n');
const writeJsonError = (io, error) => {
    io.stdout.write(stringifyJson({
        error: {
            code: error.code,
...
const writeHumanError = (io, error) => {
    io.stderr.write(`${error.message}\n`);
};
const toCliParseError = (error) => {
...
const parseFetchArguments = (args) => {
    let parsed;
    try {
        parsed = parseArgs({
            args,
    ...
    if (!parsed.values.help && !identifier) {
        throw new CliError('The fetch command requires a bit id or exact display name.', EXIT_INVALID_INPUT, 'missing-identifier');
    }
    if (parsed.values.id && positionalIdentifier) {
        throw new CliError('Provide the fetch identifier either positionally or with --id, not both.', EXIT_INVALID_INPUT, 'conflicting-identifier');
    }
    return {
        json: parsed.values.json ?? false,
        help: parsed.values.help ?? false,
...
const runFindCommand = async (args, io) => {
...
const runFetchCommand = async (args, io) => {
    ...
    if (!resolution.entry) {
        if (resolution.reason === 'ambiguous-name') {
            throw new CliError(`The identifier "${options.identifier}" matches multiple bits. Use one of: ${(resolution.matches ?? []).join(', ')}`, EXIT_INVALID_INPUT, 'ambiguous-name', { matches: resolution.matches ?? [] });
        }
...
const runMcpCommand = async (args, io) => {
...
export const runRemotionBitsCli = async (args, io = process) => {
...
//# sourceMappingURL=remotion-bits.js.map