import { createGravity, createDrag, createWiggle, createScaleOverLife, createOpacityOverLife } from "../../utils/particles/behaviors.js";
/**
...
export function getBehaviorHandlersFromProps(props) {
    const handlers = [];
    if (props.gravity)
        handlers.push(createGravity(props.gravity));
    if (props.drag !== undefined)
        handlers.push(createDrag(props.drag, props.dragVariance));
    if (props.wiggle)
        handlers.push(createWiggle(props.wiggle.magnitude, props.wiggle.frequency, props.wiggleVariance));
    if (props.scale) {
        handlers.push(createScaleOverLife(props.scale.start, props.scale.end, props.scale.startVariance, props.scale.endVariance));
    }
    if (props.opacity) {
        // Handle array shorthand [1, 0]
        if (Array.isArray(props.opacity)) {
            handlers.push(createOpacityOverLife(props.opacity, props.opacityStartVariance, props.opacityEndVariance));
        }
        else {
