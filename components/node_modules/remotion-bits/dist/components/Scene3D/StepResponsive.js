...
import { useScene3D } from "./context.js";
import { buildMotionStyles, getEasingFunction, } from "../../utils/motion/index.js";
// ============================================================================
// HELPER FUNCTIONS
// ============================================================================
/**
 ...
 */
function normalizeStepsMap(steps) {
    const map = new Map();
 ...
 */
function resolveCumulativeStepProps(stepsMap, targetIndex, sceneSteps, defaultProps) {
    const accumulatedProps = { ...defaultProps };
 ...
 */
function mergeStepValues(fromVal, toVal) {
 ...
 * ```
 */
export const StepResponsive = ({ steps, transition, children, defaultProps = {}, animate = true, centered = false, style, }) => {
    ...
    const currentStepSpecificProps = React.useMemo(() => {
        ...
        if (step?.id && stepsMap.has(step.id))
            return stepsMap.get(step.id);
        if (stepsMap.has(activeStepIndex))
            return stepsMap.get(activeStepIndex);
        return undefined;
    }, [stepsMap, activeStepIndex, sceneSteps]);
    // Get target (current active) props
    const targetStepProps = React.useMemo(() => resolveCumulativeStepProps(stepsMap, activeStepIndex, sceneSteps, defaultProps), [stepsMap, activeStepIndex, sceneSteps, defaultProps]);
    // Get previous step props for interpolation
    const prevStepIndex = activeStepIndex - 1;
    const prevStepProps = React.useMemo(() => resolveCumulativeStepProps(stepsMap, prevStepIndex, sceneSteps, defaultProps), [stepsMap, prevStepIndex, sceneSteps, defaultProps]);
    ...
    const easingFn = React.useMemo(() => getEasingFunction(effectiveConfig.easing), [effectiveConfig.easing]);
    ...
    const progress = React.useMemo(() => {
        ...
        const isActiveDurationOverride = currentStepSpecificProps?.duration !== undefined ||
    ...
    const motionStyle = React.useMemo(() => {
        ...
        transformKeys.forEach((key) => {
            ...
            const fromVal = prevStepProps[key];
            const toVal = targetStepProps[key];
            if (fromVal !== undefined && toVal !== undefined) {
                transforms[key] = mergeStepValues(fromVal, toVal);
            }
            else if (toVal !== undefined) {
                transforms[key] = toVal;
            }
            else if (fromVal !== undefined) {
                transforms[key] = fromVal;
            }
        });
        ...
        visualKeys.forEach((key) => {
            ...
            const toVal = targetStepProps[key];
            if (fromVal !== undefined && toVal !== undefined) {
                styles[key] = mergeStepValues(fromVal, toVal);
            }
            else if (toVal !== undefined) {
                styles[key] = toVal;
            }
 ...
 * ```
 */
export function useStepResponsive(steps, config) {
    const frame = useCurrentFrame();
    ...
    const targetStepProps = React.useMemo(() => resolveCumulativeStepProps(stepsMap, activeStepIndex, sceneSteps, {}), [stepsMap, activeStepIndex, sceneSteps]);
    const prevStepIndex = activeStepIndex - 1;
    const prevStepProps = React.useMemo(() => resolveCumulativeStepProps(stepsMap, prevStepIndex, sceneSteps, {}), [stepsMap, prevStepIndex, sceneSteps]);
    // Resolve effective config values
    const effectiveConfig = React.useMemo(() => {
...
//# sourceMappingURL=StepResponsive.js.map